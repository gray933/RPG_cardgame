...
export function useBattle({ playerDeckData, enemyDeckData, triggerPopup, onBack }) {
    ...
    useEffect(() => {
        ...
        if (playerGoldCount >= 5) { setGameState('win'); triggerPopup("💰 金塊が5枚揃った 特殊勝利 💰"); }

        const enemyGoldCount = enemyHand.filter(card => card && card.name === "金塊").length;
        if (enemyGoldCount >= 5) { setGameState('lose'); triggerPopup("💀 相手の金塊が5枚揃ってしまった！ 💀"); }
            ...
            case "generate_card_to_field": {
                ...
                try {
                    ...
                    if (!snap.empty) {
                        ...
                        if (isPlayerContext && playerField.length < 5) { triggerPopup(`✨ [${targetName}] が戦場に生成された！`); setPlayerField(prev => [...prev, generatedCard]); }
                        else if (!isPlayerContext && enemyField.length < 5) { triggerPopup(`👹 敵の陣地に [${targetName}] が生成された！`); setEnemyField(prev => [...prev, generatedCard]); }
                    }
                } catch (error) { console.error(error); }
                break;
            }
